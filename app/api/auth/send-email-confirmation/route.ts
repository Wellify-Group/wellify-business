// app/api/auth/send-email-confirmation/route.ts
// Отправляет письмо подтверждения email через Resend
...
// Получаем URL фронтенда для ссылки подтверждения
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (insertError) {
      console.error('[send-email-confirmation] Error inserting token:', insertError);
      return NextResponse.json(
        { 
          success: false, 
    ...
    // Формируем ссылку подтверждения (будет обрабатываться через Vercel API)
    const frontendUrl = getFrontendUrl();
    const confirmationLink = `${frontendUrl}/api/auth/confirm-email?token=${token}`;

    // Отправляем письмо через Resend
    const resendApiKey = process.env.RESEND_API_KEY;
    if (!resendApiKey) {
      console.error('[send-email-confirmation] RESEND_API_KEY is not set');
      return NextResponse.json(
    ...
    const emailFrom = process.env.RESEND_FROM_EMAIL || 'wellifybusiness@wellifyglobal.com';
    ...
    if (!resendResponse.ok) {
      ...
      console.error('[send-email-confirmation] Resend API error:', errorText);
      return NextResponse.json(
    ...
    console.log('[send-email-confirmation] ✅ Email sent successfully to:', email);
    ...
    console.error('[send-email-confirmation] Unexpected error:', err);
    return NextResponse.json(
      { 
        success: false, 
