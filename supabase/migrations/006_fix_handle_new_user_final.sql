-- Финальное исправление функции handle_new_user для заполнения профиля из метаданных
...
-- Эта функция будет вызываться автоматически при создании нового пользователя в auth.users
...
BEGIN
  ...
  -- Поддерживаем оба варианта: birth_date и dob для обратной совместимости
  v_first_name := NEW.raw_user_meta_data->>'first_name';
  v_last_name := NEW.raw_user_meta_data->>'last_name';
  v_middle_name := NEW.raw_user_meta_data->>'middle_name';
  
  -- Обрабатываем дату рождения (поддерживаем birth_date и dob)
  ...
  -- Формируем full_name из компонентов имени
  v_full_name := trim(
...
-- 6. Синхронизируем существующие записи (опционально)
...
-- 2. Заполнятся first_name, last_name, middle_name, birth_date, full_name из метаданных
